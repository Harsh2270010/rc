...
API_KEY = "bro"  # Updated API key
API_TYPE = "vehicle_num"
RETRIES = 2

# Response keys we'll extract and display
RESPONSE_KEYS = [
    "vnum", "mobile_no", "challan_status", "challan_amount",
    "vehicle_status", "owner_name", "vehicle_type", "cached", "attempt"
]

HEADERS_STATIC = [
    "#", "Vehicle No", "Status", "Mobile", "Owner", 
    "Challan Status", "Challan Amount", "Vehicle Type", "Insurance Expiry"
]
    ...
    for attempt in range(RETRIES + 1):
        try:
            ...
            if r.status_code == 200:
                ...
                # Parse challan data (can be list or dict)
                challan_data = challan_info.get("data", [])
                if isinstance(challan_data, list) and len(challan_data) > 0:
                    challan_record = challan_data[0]
                    challan_amount = challan_record.get("challan_amount", "")
                    challan_status = challan_record.get("challan_status", "")
                    challan_no = challan_record.get("challan_no", "")
                else:
                    challan_amount = ""
                    challan_status = ""
                    challan_no = ""
                
                # Parse vehicle info (should be dict)
                vehicle_data = vehicle_info.get("data", {})
                if isinstance(vehicle_data, dict):
                    owner_name = vehicle_data.get("owner_name", "")
                    vehicle_type = vehicle_data.get("vh_class", "")
                    insurance_expiry = vehicle_data.get("insurance_upto", "")
                    vehicle_color = vehicle_data.get("vehicle_color", "")
                    vehicle_status = vehicle_data.get("status", "")
                    maker_modal = vehicle_data.get("maker_modal", "")
                else:
                    owner_name = ""
                    vehicle_type = ""
                    insurance_expiry = ""
                ...
                # RC info from challan_info
                rc_info = challan_info.get("rc_info", {})

                if isinstance(rc_info, dict):
                    rc_owner = rc_info.get("owner_name", "")
                    rc_brand = rc_info.get("model_brand", "")
                else:
                    rc_owner = ""
                    rc_brand = ""
                ...
                return {
                    ...
                    "challan_response_code": challan_info.get("response_code", 0),
                    # Vehicle details
                    "vehicle_status": vehicle_status,
                    "owner_name": owner_name or rc_owner,
                    "vehicle_type": vehicle_type or maker_modal,
                    "insurance_expiry": insurance_expiry,
                    "vehicle_color": vehicle_color,
                    "vehicle_response_code": vehicle_info.get("response_code", 0),
                    # RC info
                    "rc_owner": rc_owner,
                    "rc_brand": rc_brand,
                    "_raw": data
...
def build_excel(rows: list) -> io.BytesIO:
    ...
    ok_fill   = PatternFill("solid", start_color="D4EDDA", end_color="D4EDDA")  # Green for FOUND
    emp_fill  = PatternFill("solid", start_color="F5F5F5", end_color="F5F5F5")  # Gray for NOT_FOUND
    err_fill  = PatternFill("solid", start_color="F8D7DA", end_color="F8D7DA")  # Red for ERROR
    ...
    for rd in rows:
        ...
        ws.cell(row=r, column=5, value=rd.get("owner", ""))
        ws.cell(row=r, column=6, value=rd.get("challan_status", ""))
        ws.cell(row=r, column=7, value=rd.get("challan_amount", ""))
        ws.cell(row=r, column=8, value=rd.get("vehicle_type", ""))
        ws.cell(row=r, column=9, value=rd.get("insurance_expiry", ""))
        ...
        for col_idx in range(1, len(headers) + 1):
            ...
            c.fill = fill
            c.border = border
            # Center align for first 3 columns, left align for rest
...
def parse_prefix(raw: str):
    ...
    Handles: 'UP32QP' or 'UP32QP0001' or 'UP 32 QP 0001'
    """
...
# ─── PAGE CONFIG ─────────────────────────────────────────────────────────────
st.set_page_config(page_title="Vehicle Data Fetcher v4", page_icon="🚗", layout="wide")
...
<div class="hero">
  ...
  <p>Query vehicle registration data with challan and insurance details</p>
</div>
""", unsafe_allow_html=True)
...
raw_prefix = st.text_input(
    "", value="UP32QP",
    placeholder="e.g. UP32QP or RJ02BJ or DL01AB",
...
if start_btn and not st.session_state.running and start <= end:
    ...
    for i, num in enumerate(range(start, end + 1), 1):
        ...
        has_any_data = any([
            ...
            data.get("challan_status"),
            data.get("challan_amount"),
            data.get("mobile_no")
        ])
        ...
        if has_any_data:
            ...
            log_status = f"FOUND | {data.get('owner_name','')} | ₹{data.get('challan_amount','')}"
