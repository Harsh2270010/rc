"""
RC Data Fetcher v2 - Streamlit UI for profilework239 API
Run: streamlit run rc_fetcher_v2.py
"""
...
# ─── CONSTANTS ───────────────────────────────────────────────────────────────
BASE_URL = "https://zero-vehicle2.profilework239.workers.dev/"
RETRIES  = 2

# Keys we expect from the API response (from response.response object)
RESPONSE_KEYS = [
    "regNo", "regDate", "regAuthority",
    "owner", "ownerFatherName", "financerName",
    "presentAddress", "permAddress",
    "manufacturer", "vehicle", "variant",
    "fuelType", "vehicleClass",
    "puccNumber", "puccValidUpto",
    "chassis", "engine",
    "cubicCapacity", "seatCapacity",
    "insuranceCompanyName", "insurancePolicyNumber", "insuranceUpto",
    "vehicleType", "pincode",
    "manufacturerMonthYear", "manufacturerYear", "vehicleAge",
    "rtoCode", "rtoId", "status", "statusDesc",
    "eDate", "lmDate", "transKey",
]

HEADERS = ["#", "Vehicle No", "Fetch Status"] + RESPONSE_KEYS

# ─── HELPERS ─────────────────────────────────────────────────────────────────

def fetch_vehicle(vehicle_no: str) -> dict:
    """Fetch vehicle data from the API"""
    url = f"{BASE_URL}?v={vehicle_no}"
    for attempt in range(RETRIES + 1):
        try:
            ...
            if r.status_code == 200:
                ...
                # API structure: {statusCode: 200, message: "...", response: {rtoData: {...}, regNo: "...", ...}}
                ...
                # statusCode 200 but no response data = record not found
                elif status_code == 200 and not response_data:
                    return {"_fetch_status": "NO_DATA"}
                # statusCode != 200 = API error
                else:
                    return {"_fetch_status": f"API: {message[:40]}"}
            else:
                return {"_fetch_status": f"HTTP {r.status_code}"}
        except requests.RequestException as e:
            if attempt == RETRIES:
                return {"_fetch_status": f"Error: {str(e)[:40]}"}
            time.sleep(0.5)
...
def flatten(data: dict) -> dict:
    ...
    row = {k: data.get(k, "") for k in RESPONSE_KEYS}
    return row
...
def build_excel(rows: list) -> io.BytesIO:
    ...
    # Column widths
    widths = {"#": 6, "Vehicle No": 16, "Fetch Status": 14}
    for col_idx, h in enumerate(HEADERS, 1):
...
def parse_prefix(raw: str):
    ...
    Handles: 'MP16CB' or 'MP16CB6745' or 'MP 16 CB 6745'
    """
...
# ─── PAGE CONFIG ─────────────────────────────────────────────────────────────
st.set_page_config(page_title="RC Data Fetcher v2", page_icon="🚗", layout="wide")
...
.log-ok    { color: #4caf50; font-weight: 600; }
.log-empty { color: #78909c; font-weight: 500; }
...
.stNumberInput, .stSlider, .stTextInput, .stRadio {
    font-family: 'Inter', sans-serif;
}
...
st.markdown("""
<div class="hero">
  <h1>🚗 Vehicle RC Fetcher</h1>
  <p>Query registration data by vehicle number and export results to Excel</p>
</div>
""", unsafe_allow_html=True)
...
with col_a:
    ...
    raw_prefix = st.text_input(
        "", value="MP16CB",
        placeholder="e.g. MP16CB or MP16CB6745 or DL01AB",
...
with col_b:
    ...
    digits = st.radio("", [4, 3], horizontal=True, label_visibility="collapsed",
                      help="Most series use 4 digits (0001–9999)")
    st.markdown(
        f'<div class="preview-box">🔢 Format: <b>{prefix}{"0" * digits}</b> … <b>{prefix}{"9" * digits}</b></div>',
        unsafe_allow_html=True
    )
...
if start_btn and not st.session_state.running and start <= end:
    ...
    for i, num in enumerate(range(start, end + 1), 1):
        ...
        if "_fetch_status" in data:
            status_msg = data["_fetch_status"]
            
            # Categorize error types
            if status_msg == "NO_DATA":
                status = "EMPTY"
                empty_count += 1
                css = "log-empty"
            else:
                status = status_msg[:25]
                err_count += 1
                css = "log-err"
            
            row_data = {"serial": i, "row_idx": i + 1, "vehicle_no": vehicle_no,
                        "status": status, "flat": {}}
        elif not data.get("regNo") and not data.get("engine"):
            status = "EMPTY"
            empty_count += 1
            css = "log-empty"
            row_data = {"serial": i, "row_idx": i + 1, "vehicle_no": vehicle_no,
            ...
            ok_count += 1
            css = "log-ok"
            owner = (data.get("owner") or "").strip() or "—"
            model = (data.get("vehicle") or "").strip() or "—"
            status = "OK"
            log_status = f"OK  |  {owner[:20]}  ·  {model[:20]}"
            row_data = {"serial": i, "row_idx": i + 1, "vehicle_no": vehicle_no,
                        "status": "OK", "flat": flatten(data)}
            st.session_state.results.append(row_data)

        log_lines.append(f'<span class="{css}">[{i:04d}/{total}] {vehicle_no}  →  {log_status if css == "log-ok" else status}</span>')
        ...
        if css != "log-ok" and css != "log-empty":
            st.session_state.results.append(row_data)
